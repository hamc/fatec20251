...
def add_item():
    ...
    cart.add(product_id)
    return jsonify({"message": f"Product {product_id} added to cart.", "success": True}), 201
...
def remove_item():
    ...
    if product_id in cart:
        ...
        return jsonify({"message": f"Product {product_id} removed from cart.", "success": True}), 200
    return jsonify({"message": f"Product {product_id} not found in cart.", "success": False}), 404
